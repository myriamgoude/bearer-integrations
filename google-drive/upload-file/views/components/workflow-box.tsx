...
export class WorkflowBox {
  ...
  renderFooter = () => {
    return (
      <div>
        {this.showSaveButton ? (
          <button class='save-button' onClick={() => this.onSaveClicked(this.selectedFolder)}>
            Save here
          </button>
        ) : null}
        <p class='footer-text'>
          Powered by <strong>Bearer.sh</strong>
        </p>
      </div>
    )
  }
      ...
      <div class='wrapper'>
        <shadow-box styles={{ position: 'absolute', padding: '8px 32px' }}>
          <div style={{ margin: '8px 0 32px' }}>
            <div class='settings'>
              {this.onMenu ? (
                <button class='menu' onClick={this.onMenu}>
                  <IconSettings />
                </button>
              ) : null}
              {this.onClose ? (
                <button class='menu' onClick={this.onClose}>
                  <IconClose />
                </button>
              ) : null}
            </div>
            <div class='title'>
              ...
              <div style={{ flex: '1' }}>
                <span style={{ fontSize: '16px', lineHeight: '19px', fontWeight: 'bold', color: '#030D36' }}>
                  {this.heading}
                </span>
                <span
                  style={{
